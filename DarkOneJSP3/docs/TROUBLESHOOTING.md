...
background to follow the active Columns UI background colour, or select an
explicit colour for that page. Each JScript Panel page remembers its own value.
The Playlists page and other non-JScript child panels use their own appearance
settings.
...
The supplied helper converts only at the native picker boundary. An `Overflow`
message indicates mixed files from an older test build.

Current builds log contextual messages beginning with `[DarkOneJSP3]` if a
native JScript Panel or JSplitter picker throws. Cancelling the Windows colour dialog preserves the previous mode and colour.
Selecting the Custom colour radio item itself does not open the picker; it
...
- The scrollbar thumb follows the latest mouse position while the playlist uses
  time-based interpolation at the selected UI refresh interval.
...
- Automatic follows the fastest live refresh interval reported by JS Playlist
  and Smooth Playlist Manager for the knob preview only.
- The first drag value remains immediate and the exact final value is applied
  when the mouse button is released.
- If pausing persists, include the selected visualiser settings and component
...
- If the saved anchor playlist was deleted, the manager falls back to the saved
  row and clamps it to the current list.
...
`plman.GetPlaybackQueueContents()`, so the project wrapper should not perform a
playlist-wide `%queue_indexes%` scan on first display.

If the bridge file is unavailable, invalid or explicitly reports that direct
queue enumeration failed, the viewer automatically falls back to the standalone
incremental scanner. That fallback still prioritises likely source playlists,
publishes results progressively and stops early for non-empty queues when
`%queue_total%` confirms that all queue positions have been found. An empty
queue cannot be proven without a complete fallback scan.

If a DarkOneJSP3 installation unexpectedly logs `Scanned ... items across ...
playlists`, confirm that DOJSP3.Root is using the supplied current root script
...
If those commands are absent, confirm that the panel is loading the project
wrapper rather than the generic `samples\Queue Viewer.txt` entry. The generic
Enhanced Sample Library entry is intentionally read-only outside DarkOneJSP3.

If the commands are present but a mutation is rejected, the queue probably
changed before JSplitter consumed the command. The bridge intentionally rejects
stale generations rather than applying an operation to the wrong queue. The
viewer refreshes the authoritative state automatically; retry the command after
that refresh.

If double-clicking a queued row clears the whole queue, the panel is running an
older Queue Viewer script or the source-play command was selected. In the current
project wrapper, double-click and Skip to this track remove only preceding queue
...
panel and input field should continue the surrounding gradient rather than show
one flat colour. Reload DOJSP3.QuickSearch together with the current Bottom
...
UI) and disable Draw window border. DarkOneJSP3 opts the DOJSP3.Waveform child
into JSplitter `SupportPseudoTransparency` so the host backing should show
through instead of black. Confirm that Preferences > Components reports
...
DarkOneJSP3-specific plugin message or normal polling timer. If native repaint
subscription is unavailable, its automatic guarded 100 ms fallback should
...
the source itself contains no paragraph breaks.
...
streaming/account prompts, navigation, section labels and footer text.

Known page-shell caches are removed automatically the next time the album is
loaded. If a manually edited or differently formatted stale cache remains, make
AllMusic the active source and choose Clear current source cache, then Refresh
...
background colour. This option does not affect the lower control-panel dividers.

Do not use a divider menu added to the generic Album Art JScript Panel. That
...
open Custom image path.... A relative path is resolved beneath the active
foobar2000 profile directory; use a complete path when the image is elsewhere.

The image is deliberately subtle: it is cover-cropped below the page header and
...
is empty. An active request is not duplicated; a failed or empty request can be
retried up to three times with at least 30 seconds between attempts. Download
now remains available for an immediate manual attempt.

No images downloaded means the local artist folder is empty and acquisition is
not currently active. The image region centres Downloading images... while
either the Last.fm page or its image files are still being retrieved. No images
found - retrying... means a successful empty result will be tried again
automatically; Image download failed - retrying... or Image download failed
identifies a request, parsing or file failure rather than a confirmed empty
artist page. Open the JScript Panel console for request starts, returned image
counts, queued/saved paths, errors and the final saved/failed summary. Last.fm is
temporarily unavailable - retrying... identifies a transport, 408/425/429 or 5xx
failure before gallery parsing.

An HTTP 502 response with a 136-character body is Last.fm's minimal `502 Bad
Gateway` page. It means the CDN could not obtain the artist gallery from the
upstream service; it does not mean the artist has no images. The console reports
the HTTP reason, response length, selected Content-Type/Last.fm/CDN headers and a
short sanitised preview such as `502 Bad Gateway`. It remains eligible for the
bounded automatic retry cadence.

If the console reports `unrecognised image page`, Last.fm returned a response
with a successful HTTP status that did not contain a readable gallery—commonly a
temporary verification, consent or truncated page. Non-2xx responses are
reported separately and never enter the gallery parser. Neither result is
treated as proof that the artist has no images. The downloader retries eligible
failures automatically when attempts remain and also uses a raw-markup fallback
when the gallery is present but DOM conversion failed.

Repeated Download now clicks do not queue duplicate work while the same artist
request is active. If Last.fm successfully returns no usable images, the panel
shows No images available. Enable Hide if no images available to give that
...
both use samples\shared\performance_utils.js. Older saved entry scripts are
