...
immediately during playback, without a DarkOneJSP3-specific plugin message or
...
5. Confirm that both upper dividers (Black by default) and the lower dividers
   align.
6. Confirm that the display and waveform occupy the lower centre area.
7. Confirm `DOJSP3.QuickSearch` is a JScript Panel 3 instance loading the
   DarkOneJSP3 Quick Search wrapper.
8. Confirm Enhanced Spectrum Analyser uses the component preferences documented
   above.
9. Confirm Waveform Minibar uses the component preferences documented above.
10. Configure an optional TOOLS control-panel button with the internal command
   DarkOneJSP3/Tools/Menu and confirm that the DarkOne Tools popup opens below
   the clicked button.
