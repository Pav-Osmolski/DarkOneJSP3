...
function createSettingsBackArrow(colour, size) {
	...
	// Rasterise one closed silhouette. Every destination pixel is written at
	// most once, so semi-transparent hover colours cannot become darker where
	// the arrowhead meets the shaft. Four-by-four coverage sampling keeps the
	// diagonals clean without composing overlapping line strokes.
	for (var y = 0; y < size; y++) {
		for (var x = 0; x < size; x++) {
			var covered = 0;
			for (var sy = 0; sy < samplesPerAxis; sy++) {
				for (var sx = 0; sx < samplesPerAxis; sx++) {
					var px = (x + (sx + 0.5) / samplesPerAxis) / size;
					var py = (y + (sy + 0.5) / samplesPerAxis) / size;
					if (containsPoint(px, py)) covered++;
				}
			}
			if (covered > 0) {
				var alpha = Math.round(baseAlpha * covered / sampleCount);
...
function oSettings() {
	...
	this.setButtons = function () {
		...
		var close_off = createSettingsBackArrow(this.color2, button_zoomSize);
		var close_ov = createSettingsBackArrow(this.color1, button_zoomSize);
